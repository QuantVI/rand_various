# -*- coding: utf-8 -*-
"""
Created on Sun Aug 31 23:42:00 2025
"""
...
# 1. run model
  ...
  # o. apply any needed currency conversion and capping
  # p. output relative and absolute predictions
  # q. output market data and transformations
  # r. run data quality/ KPI checks if flag for this was TRUE

import a04_connect_data as connector

# Since this will be called via an orchestrator,
# we can write this as if __main__ is the orchestrator.

    # a) Handled by orchestrator: read config
    # b) Connect to the Database
        # - connection and extraction handled in on step
    # c) Pull data
def extract_portfolio(some_dates, typ="Portfolio"):
    """
    The call to conn_to_db will prompt the user
    for their username and password via pop-up dialog box.
    """
    # ---- create conn and return conn object
    ...
    # f) Pull Market data from now back x months
...
NOTE_01= """
import [4] connect-data
--> get_marketdata

GIVEN [2].spec
GIVEN [4].extract_portf
GIVEN [1].model-vars

Transform mrktdta
Array func regression formula
Project forward
Apply curr convert
Return proj
...
NOTE_04 = """
    work backwards, where function/method calls are single-statmements
    that will encapsulate many other steps and context.
    """
